...
Mesh во время обучения берётся из `scene_dir/mesh_support/*.npz`; если кеш неполный, `train.py` падает и просит запустить precompute. Live-рендер остаётся только за явным `--force_live_mesh`.

## Precompute mesh

```bash
python precompute_mesh_support.py \
  --scene_dir ../scene_parus \
  --mesh_obj ../scene_parus/parus.obj
```
...
Главные флаги: `--mesh_support_dir`, `--force_live_mesh`, `--init_means_ckpt`, `--init_points`, `--max_steps`, `--eval_every`, `--save_every`, `--prune_opa`, `--opacity_prune_every`, `--opacity_prune_log_every`, `--gate_eps`, `--gate_band`.
